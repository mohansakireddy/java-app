            ...
            steps{
                script{
                    withCredentials([string(credentialsId: 'docker_pass', variable: 'docker_password')]) {
                             sh '''
                                docker build -t 18.177.150.61:8083/springapp:${VERSION} .
                                docker login -u admin -p $docker_password 18.177.150.61:8083 
                                docker push  18.177.150.61:8083/springapp:${VERSION}
                                docker rmi 18.177.150.61:8083/springapp:${VERSION}
                            '''
                    }
                }
            }
...
//
   post {
   	always {
   		mail bcc: '', body: "<br>Project: ${env.JOB_NAME} <br>Build Number: ${env.BUILD_NUMBER} <br> URL de build: ${env.BUILD_URL}", cc: '', charset: 'UTF-8', from: '', mimeType: 'text/html', replyTo: '', subject: "${currentBuild.result} CI: Project name -> ${env.JOB_NAME}", to: "mohansaki01@gmail.com";  
   	 }
	  }
}//
//